const translations = {
    en: {
        // Navigation
        "nav_about": "About",
        "nav_mission": "Mission",
        "nav_products": "Products",
        "nav_blog": "Daily AI Blog",
        "nav_submit": "Submit Idea",
        "nav_fellowship": "Fellowship",
        "nav_contact": "Get Involved",

        // Hero
        "hero_title": "Building AI Careers,<br>Transforming Social Impact",
        "hero_subtitle": "Empowering underserved youth with AI career pathways while delivering cutting-edge AI solutions to NGOs tackling critical challenges in health, education, and agriculture.",
        "hero_btn_learn": "Learn More →",
        "hero_btn_submit": "Submit an Idea",
        "hero_btn_involved": "Get Involved",
        "hero_btn_fellowship": "Join Fellowship",

        // Stats
        "stat_engineers": "Engineers Teamed",
        "stat_products": "AI Products being Explored",
        "stat_ngos": "NGO collaborators being pursued",
        "stat_runway": "Years Secured Runway",

        // About
        "about_title": "Bridging Two Critical Gaps",
        "about_subtitle": "We're solving two interconnected problems that prevent technology from reaching those who need it most",
        "impact_talent_title": "🎓 The Talent Gap",
        "impact_talent_desc": "Graduates from low-income families lack access to high-quality AI/ML career pathways and mentorship. Despite their talent and drive, systemic barriers prevent them from entering the AI field.",
        "impact_talent_sol": "<strong>Our Solution:</strong> Create structured career pathways with mentorship, training, and real-world project experience that transforms promising graduates into skilled AI engineers.",
        "impact_tech_title": "💻 The Technology Gap",
        "impact_tech_desc": "NGOs struggle with inefficiencies—manual paperwork, lack of data infrastructure—but cannot afford expensive AI solutions. They need technology but lack resources to implement it.",
        "impact_tech_sol": "<strong>Our Solution:</strong> Deliver enterprise-grade AI products at no cost or minimal cost to NGOs, enabling them to amplify their impact and serve more people effectively.",

        // Mission
        "mission_title": "Our Mission",
        "mission_subtitle": "Building a sustainable AI innovation center that creates careers and delivers impact",
        "mission_talent_title": "Talent Development",
        "mission_talent_desc": "Provide livelihoods and advanced AI career pathways for underprivileged graduates and engineers who have struggled to find stable employment. We invest in people with potential, not just credentials.",
        "mission_product_title": "Product Innovation",
        "mission_product_desc": "Build and deploy AI products that solve urgent, specific problems for NGOs in health, education, and agriculture. Every product we create directly improves lives in underserved communities.",
        "mission_impact_title": "Sustainable Impact",
        "mission_impact_desc": "Establish a foundation-funded, non-profit model with 2-3 year secured runway. IP owned by US entity; social-use license offered free or at-cost to NGOs globally.",

        // Products
        "products_title": "AI Products for Social Good",
        "products_subtitle": "Practical AI solutions addressing real-world challenges faced by NGOs and social workers",

        // Blog
        "blog_title": "Daily AI Research",
        "blog_subtitle": "Exploring the latest AI applications in social service, agriculture, health, and education from around the globe.",
        "read_more": "Read Full Article →",

        // Form
        "form_title": "Have an AI Product Idea?",
        "form_subtitle": "We're looking for impactful AI product ideas that can solve real problems for NGOs and underserved communities. Share your vision with us!",
        "label_name": "Your Name *",
        "label_email": "Email Address *",
        "label_org": "Organization (if applicable)",
        "label_sector": "Social Impact Sector *",
        "label_problem": "What Problem Does This Solve? *",
        "label_solution": "Your AI Product Idea *",
        "label_impact": "Expected Impact *",
        "label_data": "Data Availability",
        "btn_submit_idea": "Submit Your Idea",

        // Footer
        "footer_about_title": "About VitaInspire",
        "footer_about_desc": "Building AI careers for underserved youth while delivering impactful AI products for NGOs in health, education, and agriculture.",
        "footer_quick_links": "Quick Links",
        "footer_fellowship": "Join Fellowship",
        "footer_get_involved": "Get Involved",
        "footer_contact": "Contact",
        "footer_rights": "VitaInspire. All rights reserved. Built with ❤️ for social impact."
    },
    hi: {
        // Navigation
        "nav_about": "हमारे बारे में",
        "nav_mission": "हमारा मिशन",
        "nav_products": "उत्पाद",
        "nav_blog": "दैनिक एआई ब्लॉग",
        "nav_submit": "विचार भेजें",
        "nav_fellowship": "फेलोशिप",
        "nav_contact": "जुड़ें",

        // Hero
        "hero_title": "एआई करियर का निर्माण,<br>सामाजिक प्रभाव का परिवर्तन",
        "hero_subtitle": "वंचित युवाओं को एआई करियर के रास्ते प्रदान करना और साथ ही स्वास्थ्य, शिक्षा और कृषि में महत्वपूर्ण चुनौतियों से निपटने वाले एनजीओ को अत्याधुनिक एआई समाधान देना।",
        "hero_btn_learn": "और जानें →",
        "hero_btn_submit": "एक विचार भेजें",
        "hero_btn_involved": "जुड़ें",
        "hero_btn_fellowship": "फेलोशिप में शामिल हों",

        // Stats
        "stat_engineers": "इंजीनियर्स की टीम",
        "stat_products": "एआई उत्पादों की खोज",
        "stat_ngos": "एनजीओ सहयोगी",
        "stat_runway": "वर्षों की सुरक्षित फंडिंग",

        // About
        "about_title": "दो महत्वपूर्ण कमियों को पूरा करना",
        "about_subtitle": "हम दो परस्पर जुड़ी समस्याओं को हल कर रहे हैं जो तकनीक को उन लोगों तक पहुँचने से रोकती हैं जिन्हें इसकी सबसे अधिक आवश्यकता है",
        "impact_talent_title": "🎓 प्रतिभा की कमी",
        "impact_talent_desc": "कम आय वाले परिवारों के स्नातकों के पास उच्च गुणवत्ता वाले एआई/एमएल करियर और मार्गदर्शन की कमी है। उनकी प्रतिभा और उत्साह के बावजूद, प्रणालीगत बाधाएं उन्हें एआई क्षेत्र में प्रवेश करने से रोकती हैं।",
        "impact_talent_sol": "<strong>हमारा समाधान:</strong> मार्गदर्शन, प्रशिक्षण और वास्तविक दुनिया के प्रोजेक्ट अनुभव के साथ संरचित करियर पथ बनाना जो होनहार स्नातकों को कुशल एआई इंजीनियरों में बदल देता है।",
        "impact_tech_title": "💻 तकनीकी कमी",
        "impact_tech_desc": "एनजीओ अक्षमताओं—मैनुअल कागजी कार्रवाई, डेटा बुनियादी ढांचे की कमी—से जूझते हैं लेकिन महंगे एआई समाधानों का खर्च नहीं उठा सकते। उन्हें तकनीक की आवश्यकता है लेकिन इसे लागू करने के लिए संसाधनों की कमी है।",
        "impact_tech_sol": "<strong>हमारा समाधान:</strong> एनजीओ को बिना किसी लागत या न्यूनतम लागत पर उद्यम-श्रेणी के एआई उत्पाद प्रदान करना, जिससे वे अपने प्रभाव को बढ़ा सकें और अधिक लोगों की प्रभावी ढंग से सेवा कर सकें।",

        // Mission
        "mission_title": "हमारा मिशन",
        "mission_subtitle": "एक स्थायी एआई नवाचार केंद्र का निर्माण जो करियर बनाता है और प्रभाव डालता है",
        "mission_talent_title": "प्रतिभा विकास",
        "mission_talent_desc": "वंचित स्नातकों और इंजीनियरों के लिए आजीविका और उन्नत एआई करियर के रास्ते प्रदान करना जो स्थिर रोजगार खोजने के लिए संघर्ष कर रहे हैं। हम केवल डिग्री में नहीं, बल्कि क्षमता वाले लोगों में निवेश करते हैं।",
        "mission_product_title": "उत्पाद नवाचार",
        "mission_product_desc": "एआई उत्पादों का निर्माण और तैनाती जो स्वास्थ्य, शिक्षा और कृषि में एनजीओ के लिए तत्काल, विशिष्ट समस्याओं को हल करते हैं। हमारे द्वारा बनाया गया हर उत्पाद सीधे वंचित समुदायों में जीवन को बेहतर बनाता है।",
        "mission_impact_title": "स्थायी प्रभाव",
        "mission_impact_desc": "2-3 साल की सुरक्षित फंडिंग के साथ एक फाउंडेशन-वित्तपोषित, गैर-लाभकारी मॉडल स्थापित करना। यूएस इकाई के स्वामित्व वाले आईपी; एनजीओ को विश्व स्तर पर मुफ्त या लागत पर सामाजिक उपयोग लाइसेंस।",

        // Products
        "products_title": "सामाजिक भलाई के लिए एआई उत्पाद",
        "products_subtitle": "एनजीओ और सामाजिक कार्यकर्ताओं द्वारा सामना की जाने वाली वास्तविक दुनिया की चुनौतियों को संबोधित करने वाले व्यावहारिक एआई समाधान",

        // Blog
        "blog_title": "दैनिक एआई अनुसंधान",
        "blog_subtitle": "दुनिया भर से सामाजिक सेवा, कृषि, स्वास्थ्य और शिक्षा में नवीनतम एआई अनुप्रयोगों की खोज।",
        "read_more": "पूरा लेख पढ़ें →",

        // Form
        "form_title": "क्या आपके पास कोई एआई उत्पाद विचार है?",
        "form_subtitle": "हम प्रभावशाली एआई उत्पाद विचारों की तलाश कर रहे हैं जो एनजीओ और वंचित समुदायों के लिए वास्तविक समस्याओं को हल कर सकें। अपनी दृष्टि हमारे साथ साझा करें!",
        "label_name": "आपका नाम *",
        "label_email": "ईमेल पता *",
        "label_org": "संगठन (यदि लागू हो)",
        "label_sector": "सामाजिक प्रभाव क्षेत्र *",
        "label_problem": "यह किस समस्या को हल करता है? *",
        "label_solution": "आपका एआई उत्पाद विचार *",
        "label_impact": "अपेक्षित प्रभाव *",
        "label_data": "डेटा उपलब्धता",
        "btn_submit_idea": "अपना विचार जमा करें",

        // Footer
        "footer_about_title": "विटा इंस्पायर के बारे में",
        "footer_about_desc": "वंचित युवाओं के लिए एआई करियर का निर्माण करते हुए स्वास्थ्य, शिक्षा और कृषि में एनजीओ के लिए प्रभावशाली एआई उत्पाद प्रदान करना।",
        "footer_quick_links": "त्वरित लिंक",
        "footer_fellowship": "फेलोशिप में शामिल हों",
        "footer_get_involved": "शामिल हों",
        "footer_contact": "संपर्क",
        "footer_rights": "विटा इंस्पायर। सर्वाधिकार सुरक्षित। सामाजिक प्रभाव के लिए ❤️ के साथ बनाया गया।"
    }
};
